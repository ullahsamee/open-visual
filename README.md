...
- Live streaming works best when `Iframe Same-Origin Access` is enabled.
- Model-attached skill lazy loading depends on native function calling.
- `download_svg` only succeeds when the visual is a single standalone SVG root.
- Very large datasets are truncated to the limits listed above.

## Security

Open WebUI Tools and Functions execute Python on your server. Install this bundle only on instances you control, review the source before enabling it, and restrict plugin-management access to trusted administrators.
